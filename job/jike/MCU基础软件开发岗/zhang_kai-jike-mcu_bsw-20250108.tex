...
\normalsize
5年\tech{AUTOSAR}软件开发经验，负责\tech{AUTOSAR BSW}模块配置，包括\tech{SOME/IP}服务配置、诊断（\tech{UDS}）等核心模块配置，具备车身域控制器集成开发和量产经验以及中央网关计算平台集成开发经验。

% ============ 关键能力 ============
\section*{关键能力}

\begin{itemize}
  \skillitem{诊断配置}{负责\tech{UDS}诊断协议栈配置，完成\tech{Dcm}诊断通信管理、\tech{Dem}故障事件管理配置}
  \skillitem{SOME/IP配置}{负责车载以太网\tech{SOME/IP}相关模块配置，包括\tech{SoAd}传输层、\tech{SD}服务发现、\tech{PduR}路由和\tech{TCP/UDP}配置}
  \skillitem{Vector工具链}{熟练使用\tech{Vector DaVinci Developer}、\tech{DaVinci Configurator}进行\tech{AUTOSAR BSW}模块配置和代码生成，解决\tech{ARXML}配置冲突和兼容性问题}
...
\subsubsection*{SPA3/GPA高性能计算平台集成｜AUTOSAR BSW集成工程师}
\hfill \textit{2023年11月 -- 2025年7月}

\begin{itemize}
  \item[\ding{51}] \highlight{AUTOSAR BSW模块配置}：负责高性能计算平台\tech{HI}(基于Infineon TC397)的\tech{AUTOSAR BSW}基础软件模块配置，包括通信（\tech{CanIf, CanTp, PduR, ComM}）、诊断（\tech{Dem, Dcm}）、存储（\tech{NvM, MemIf}）等核心功能模块配置
  ...
  \item[\ding{51}] \highlight{SOME/IP配置}：负责车载以太网\tech{SOME/IP}相关模块配置，包括\tech{SoAd}传输层、\tech{SD}服务发现、\tech{PduR}路由和\tech{TCP/UDP}配置，使用\tech{Davinci Configurator}完成配置和代码生成
  \item[\ding{51}] \highlight{诊断配置}：负责\tech{UDS}诊断协议栈配置，完成\tech{Dcm}诊断通信管理、\tech{Dem}故障事件管理配置
  \item[\ding{51}] \highlight{存储管理与NVM配置}：负责\tech{NVM}非易失性存储管理配置，设计\tech{NvM}块管理策略、\tech{MemIf}抽象接口配置，支持用户数据、\tech{DID}数据、\tech{ adaptation parameter}等数据的持久化存储
  \item[\ding{51}] \highlight{MCAL配置与芯片初始化}：参与\tech{MCAL}层配置，包括\tech{Dio}（\tech{Digital IO}）、\tech{Port}、\tech{GPT}（\tech{General Purpose Timer}）、\tech{ICU}（\tech{Input Capture Unit}）等外设驱动配置，完成\tech{Tricore}芯片级初始化和时钟树配置
  ...
  \item[\ding{51}] \highlight{工具链开发}：主导基于\tech{Python}的\tech{yaml2arxml}转换工具和\tech{SWC}配置管理工具开发，实现\tech{AUTOSAR}配置文件的全流程自动化管理，补充\tech{Davinci Developer}配置功能
\end{itemize}
...
\begin{itemize}
  \item[\ding{51}] \highlight{车身域控制器BSW配置}：负责车身域控制器的\tech{AUTOSAR BSW}基础软件配置，主导\tech{CAN/LIN}通信协议栈配置、\tech{UDS}诊断配置、\tech{NVM}存储管理配置，完成多个量产项目的\tech{BSW}模块配置和验证
...
\begin{itemize}
  ...
  \begin{itemize}
    \item 主导\tech{AUTOSAR BSW}基础软件配置，负责\tech{CAN/LIN}通信、\tech{UDS}诊断、\tech{NVM}存储等核心模块配置，支持\tech{BLE, TBOX, VCU, ESCL}等安全关键模块的通信集成
