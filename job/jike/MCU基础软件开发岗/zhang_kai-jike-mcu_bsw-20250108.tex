...
\normalsize
5年\tech{AUTOSAR}软件开发经验，负责\tech{SOME/IP}服务配置（\tech{SoAd}、\tech{SD}、\tech{PduR}等），具备车身域控制器和中央网关计算平台集成开发经验。
...
\begin{itemize}
  ...
  \item[\ding{51}] \highlight{SOME/IP配置与测试}：负责\tech{SOME/IP}服务配置（\tech{SoAd}、\tech{SD}、\tech{BswM}），使用\tech{CANoe}配合\tech{VN5650}测试，使用\tech{劳德巴赫}调试\tech{C代码}
\end{itemize}
...
\begin{itemize}
  ...
  \item[\ding{51}] \highlight{测试程序开发}：设计生产线最终检验程序，开发\tech{EMC}上位机和\tech{PCBA}的\tech{EMC}测试专用软件，确保产品符合电磁兼容性要求
  \item[\ding{51}] \highlight{客户支持与软件发布}：定位客户问题并协调相关工程师改进软件，作为技术接口人向客户发布软件并提供维护服务，建立质量问题分派和跟踪机制
\end{itemize}
...
\begin{itemize}
  ...
  \begin{itemize}
    \item 负责新能源汽车车身域控制器的集成测试和软件发布
  \end{itemize}

  \item[\ding{108}] \textbf{江西五十铃} - \tech{PEPS}无钥匙进入系统 \& \tech{ESCL}电子转向锁项目｜集成开发负责人
  \begin{itemize}
    \item 负责\tech{PEPS}和\tech{ESCL}系统的集成测试和客户问题支持
  \end{itemize}
