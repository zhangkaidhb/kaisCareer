...
\normalsize
5年实时操作系统和\tech{AUTOSAR CP}基础软件栈开发经验，精通\tech{AUTOSAR OS}任务调度、中断管理和系统实时性优化，深入理解\tech{Infineon Tricore}、\tech{Arm-M/R}等车规级\tech{MCU}芯片架构，具备高性能中央计算平台和车身域控制器的\tech{OS}模块开发、量产软件问题定位和维测能力增强设计经验，熟练掌握嵌入式\tech{C}语言开发工具链（\tech{GCC}、\tech{Tasking}、\tech{Makefile}、\tech{Git}）。

% ============ 关键能力 ============
\section*{关键能力}

\begin{itemize}
  \skillitem{实时操作系统}{熟悉\tech{AUTOSAR OS}规范，掌握任务调度（\tech{Full/Non Preemptive}、\tech{Mix}）、中断管理（\tech{ISR Category 1/2}）、资源管理（\tech{Resource}）等核心特性，}
  \skillitem{AUTOSAR CP基础软件}{熟悉\tech{AUTOSAR Classic Platform}基础软件栈，包括\tech{BSW}模块配置（\tech{CanIf, LinIf, PduR, ComM}）、\tech{RTE}接口配置和代码生成，具备实际项目配置经验}
  \skillitem{MCU芯片架构}{熟悉\tech{Infineon Tricore}（\tech{AURIX TC3xx}系列）多核架构、\tech{Arm Cortex-M/R}系列架构，了解\tech{MCU}内存映射、时钟树配置等硬件特性}
  \skillitem{系统性能优化}{熟悉系统实时性要求，能够使用\tech{Trace32}进行性能分析和任务调试}
  \skillitem{通信协议栈}{熟悉车载通信协议，包括\tech{CAN}（\tech{Classic CAN, CAN-FD}）、\tech{LIN}、车载以太网\tech{SOME/IP}配置（\tech{SoAd}、\tech{SD}、\tech{PduR}、\tech{TCP/UDP}）、\tech{DoIP}诊断、\tech{UDS}诊断（\tech{ISO 14229}、\tech{ISO 15765}）、\tech{XCP}标定协议}
  \skillitem{编译工具链}{熟练使用\tech{GCC}、\tech{Tasking}等嵌入式编译器，掌握\tech{Makefile}构建脚本编写、\tech{Linker Script}链接脚本配置、\tech{Git}版本管理，具备交叉编译环境和自动化构建系统搭建经验}
  \skillitem{诊断与标定协议}{熟悉\tech{UDS}诊断协议（\tech{ISO 14229 UDS}、\tech{ISO 15765-2 TP}）、\tech{XCP}标定协议（\tech{ISO 15031}），负责\tech{Dem}（\tech{Diagnostic Event Manager}）和\tech{Dcm}（\tech{Diagnostic Communication Manager}）模块配置，具备\tech{DID}、\tech{RID}、\tech{DTC}配置和诊断测试经验}
...
\begin{tabularx}{\textwidth}{@{}p{3.2cm}X@{}}
  \textbf{RTOS} & \tech{AUTOSAR OS}, \tech{OSEK/VDX} \\
  ...
  \textbf{编译工具链} & \tech{GCC}, \tech{Tasking}, \tech{Makefile}, \tech{Linker Script}, \tech{Git} \\
  \textbf{通信协议} & \tech{CAN/CAN-FD}, \tech{LIN}, \tech{SOME/IP}, \tech{DoIP}, \tech{UDS}, \tech{XCP} \\
...
\begin{itemize}
  ...
  \item[\ding{51}] \highlight{AUTOSAR CP基础软件配置}：负责\tech{BSW}模块配置和\tech{MCAL}配置，包括\tech{CanIf}、\tech{LinIf}、\tech{PduR}、\tech{ComM}、\tech{Dem}、\tech{Dcm}等基础软件模块，完成\tech{ARXML}配置和代码生成，解决\tech{ECUExtract}合规检查错误和配置冲突问题
  \item[\ding{51}] \highlight{编译工具链与构建系统}：负责嵌入式\tech{C}语言交叉编译环境搭建，配置\tech{GCC}和\tech{Tasking}编译器，编写\tech{Makefile}构建脚本和\tech{Linker Script}链接脚本，优化编译选项以提升代码性能和减少\tech{Flash}/\tech{RAM}占用
  \item[\ding{51}] \highlight{诊断协议实现与测试}：负责\tech{UDS}诊断协议栈配置，实现\tech{DID}数据标识符、\tech{RID}例行标识符、\tech{DTC}诊断故障码的配置和管理，使用\tech{CANoe}进行诊断服务测试（\tech{0x10/0x22/0x2E/0x19}等），支持量产软件的故障诊断和刷写功能}
...
\begin{itemize}
  ...
  \item[\ding{51}] \highlight{软件集成与构建}：负责车身域控制器软件集成，维护\tech{Makefile}构建脚本和\tech{GCC}交叉编译环境，优化编译配置以提升构建效率，管理软件版本和需求追溯关系，确保软件符合\tech{ASPICE}流程要求
  \item[\ding{51}] \highlight{诊断模块开发}：负责车身域控制器\tech{NVM}模块开发，实现\tech{SWC}配置数据、诊断数据（\tech{DID}、\tech{RID}、\tech{DTC}）的持久化存储，使用\tech{CANoe}进行诊断服务测试验证
