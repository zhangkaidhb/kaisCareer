...
\normalsize
5年汽车嵌入式软件开发经验，熟悉车载以太网通信协议栈，具备\tech{SOME/IP}服务设计、配置和测试验证经验，负责\tech{EthTSyn}模块的\tech{IEEE 802.1AS(gPTP)}时间同步配置，了解\tech{IEEE 802.1Qbv}、\tech{IEEE 802.1Qav}等\tech{TSN}协议及\tech{QoS}保障机制，具备\tech{AUTOSAR}架构集成、车载网络测试验证和\tech{Linux}环境开发经验。

% ============ 关键能力 ============
\section*{关键能力}

\begin{itemize}
  \skillitem{车载以太网通信}{精通\tech{SOME/IP}服务完整配置流程（\tech{SoAd, SD, BswM, EcuC, ComM, PduR}），熟悉\tech{DoIP}、\tech{XCP on Ethernet}、\tech{UDP NM}等协议，了解\tech{TCP/IP}、\tech{VLAN}、\tech{IPv4/v6}等底层协议栈}
  ...
  \skillitem{以太网测试与验证}{熟练使用\tech{CANoe}配合\tech{VN5650}进行\tech{SOME/IP}服务通信测试和\tech{TC8}标准测试，熟悉\tech{Upper Tester}测试规范，具备\tech{Wireshark}抓包分析能力}
  \skillitem{AUTOSAR架构基础}{熟悉\tech{Classic AUTOSAR}架构，具备\tech{BSW}、\tech{RTE}、\tech{SWC}集成经验，掌握\tech{Davinci Configurator}配置和\tech{ARXML}文件管理}
  \skillitem{开发与调试能力}{精通\tech{Python}脚本开发和\tech{C}语言编程，熟悉\tech{Linux}环境开发，熟练使用\tech{Lauterbach}、\tech{iSYSTEM winIDEA}进行代码级调试}
\end{itemize}
...
\begin{tabularx}{\textwidth}{@{}p{3.2cm}X@{}}
  ...
  \textbf{TSN协议} & \tech{IEEE 802.1AS(gPTP)}, \tech{IEEE 802.1Qbv}, \tech{IEEE 802.1Qav}, \tech{IEEE 802.1Qci}, \tech{IEEE 802.1Qat} \\
  \textbf{测试工具} & \tech{CANoe+VN5650}, \tech{Wireshark}, \tech{TC8 Upper Tester}, \tech{vTESTstudio} \\
  \textbf{开发平台} & \tech{Linux}, \tech{AUTOSAR CP}, \tech{C}, \tech{Python} \\
...
\begin{itemize}
  \item[\ding{51}] \highlight{SOME/IP服务配置与验证}：主导车载以太网\tech{SOME/IP}服务的完整配置流程，在\tech{Davinci Configurator}中完成\tech{SoAd}(Socket Adapter)、\tech{SD}(Service Discovery)、\tech{BswM}(Mode Switch)、\tech{EcuC}、\tech{ComM}、\tech{PduR}等模块配置，负责服务发布/订阅关系的\tech{ARXML}配置管理
  ...
  \item[\ding{51}] \highlight{QoS保障机制设计}：参与\tech{TSN}协议族的\tech{QoS}保障机制设计，包括\tech{IEEE 802.1Qbv}时间感知整形(\tech{TAS})的门控列表配置、\tech{IEEE 802.1Qav}信用整形(\tech{CBS})的带宽分配配置，确保关键流量(如制动、转向)的时延保障
  \item[\ding{51}] \highlight{跨域以太网集成}：负责\tech{ZC}(NVIDIA Orin, Linux环境)与\tech{HI}(Infineon TC397, AUTOSAR CP)的异构系统以太网通信集成，解决跨域通信协议栈适配、\tech{Endianness}字节序转换、\tech{Payload}数据格式对齐等技术挑战
  \item[\ding{51}] \highlight{网络调试与问题定位}：使用\tech{Wireshark}进行以太网数据包抓包分析，结合\tech{CANoe}的\tech{TC8}测试标准进行通信协议验证，定位\tech{SOME/IP}服务发现失败、数据传输丢包、时序不满足等网络问题
\end{itemize}
