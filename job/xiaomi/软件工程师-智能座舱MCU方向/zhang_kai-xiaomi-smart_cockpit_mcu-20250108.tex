...
\normalsize
5年汽车电子软件开发经验，熟悉\tech{AUTOSAR CP}架构和智能座舱\tech{MCU}软件开发，具备\tech{SWC}应用层开发、\tech{BSW}基础软件配置集成、车身域控制器集成经验，熟练使用\tech{Vector DaVinci}工具链，熟悉\tech{CAN}、\tech{UDS}、\tech{NM}、\tech{DoIP}等座舱通信协议。
...
\begin{itemize}
  ...
  \skillitem{BSW基础软件配置}{熟练使用\tech{DaVinci Configurator}进行\tech{AUTOSAR BSW}模块配置，包括通信（\tech{CAN, LIN}）、诊断（\tech{UDS}）、存储（\tech{NVM}）、网络管理（\tech{NM}）等}
  \skillitem{Vector DaVinci工具}{使用\tech{DaVinci Developer}进行\tech{SWC}开发，使用\tech{DaVinci Configurator}进行\tech{BSW}配置}
  \skillitem{座舱通信协议}{熟悉\tech{CAN}总线通信协议、\tech{UDS}诊断协议、\tech{NM}网络管理、\tech{DoIP}等协议}
...
\begin{tabularx}{0.94\textwidth}{@{}p{2.6cm}X@{}}
  \textbf{AUTOSAR开发} & \tech{SWC应用层开发}, \tech{底层模块开发}, \tech{BSW配置集成}, \tech{RTE接口连接}, \tech{OS时序挂载} \\
...
\begin{itemize}
  \item[\ding{51}] \highlight{SWC应用层开发与配置}：使用\tech{DaVinci Developer}进行\tech{SWC}应用层开发，负责\tech{Runnable}实体设计、\tech{Data Type}定义、\tech{Sender-Receiver}和\tech{Client-Server}接口配置，完成\tech{RTE}接口连接和代码生成
  \item[\ding{51}] \highlight{BSW基础软件配置}：使用\tech{DaVinci Configurator}进行\tech{BSW}标准模块配置，包括\tech{CAN}通信（\tech{CanIf, CanTp, PduR}）、\tech{UDS}诊断（\tech{DEM, Dcm}）、\tech{NVM}存储（\tech{NvM, MemIf, Fee}）、\tech{NM}网络管理、\tech{ComM}通信管理等核心功能模块
  \item[\ding{51}] \highlight{底层模块开发}：负责基于\tech{C}语言的底层模块开发，包括\tech{Auth}安全认证模块、\tech{NvmManager}存储管理模块，实现座舱功能的底层逻辑和数据管理
