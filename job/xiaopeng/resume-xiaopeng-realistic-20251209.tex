...
\normalsize
5年汽车嵌入式软件开发经验，曾主导汽车嵌入式软件架构设计、确定性通信系统中间件配置、实时车载网络测试验证。掌握\tech{AUTOSAR CP}架构应用、\tech{TSN}网络基础配置、车载以太网通信测试，具备实际的工具配置和集成测试经验。

% ============ 关键能力 ============
\section*{关键能力}

\begin{itemize}
  \skillitem{AUTOSAR架构与系统集成}{精通\tech{Classic AUTOSAR}架构，熟悉\tech{Adaptive AUTOSAR}，具备高性能中间计算平台和车身域控制器集成经验，精通车载网络通信（\tech{CAN}/\tech{LIN}/以太网）}
  \skillitem{确定性通信中间件}{主导\tech{SOME/IP}服务配置，深入理解\tech{TSN}协议族（\tech{IEEE802.1AS gPTP}, \tech{802.1Qbv}, \tech{802.1Qav}）及\tech{QoS}保障机制，具备车载以太网通信经验}
  \skillitem{软件开发能力}{精通\tech{Python}、\tech{MATLAB Stateflow}、\tech{C}编程，具备汽车嵌入式SWC开发经验、\tech{AUTOSAR}架构能力和基于\tech{UML}语言的架构设计能力}
  \skillitem{工具链与自动化}{精通Vector工具链，具备CI/CD流水线设计和自动化脚本开发经验，熟悉\tech{Linux}环境}
\end{itemize}

\subsection*{技术技能矩阵}

\begin{tabularx}{\textwidth}{@{}p{3.2cm}X@{}}
  \textbf{编程语言} & \tech{Python}, \tech{Groovy}, \tech{C}, \tech{MATLAB(Stateflow)} \\
  \textbf{Vector工具链} & \tech{DaVinci Developer\&Configurator}, \tech{CANoe}, \tech{vTESTstudio} \\
  \textbf{CI/CD工具} & \tech{Jenkins}, \tech{Gerrit}, \tech{Gradle}, \tech{MongoDB}, \tech{Git} \\
...
\begin{itemize}
  ...
  \item[\ding{51}] \highlight{车载以太网中间件配置}：主导\tech{SOME/IP}服务在\tech{Davinci Configurator}中的完整配置流程，包含\tech{SoAd, SD, BswM, EcuC, ComM, PduR}等模块配置，负责\tech{EthTSyn}模块的\tech{IEEE802.1AS(gPTP)}时间同步配置，确保服务正确部署和确定性通信保障
  \item[\ding{51}] \highlight{通信验证与测试}：使用\tech{CANoe}配合\tech{VN5650}进行\tech{SOME/IP}服务通信测试，验证车载以太网通信机制和时序要求
  \item[\ding{51}] \highlight{系统调试与部署}：通过\tech{Linux}环境配置和劳德巴赫调试器进行\tech{C}代码级调试，保障系统集成稳定性
\end{itemize}
...
\begin{itemize}
  \item[\ding{51}] \highlight{CI/CD流水线架构与脚本开发}：支持设计企业级CI/CD流水线系统，基于\tech{Gerrit-Jenkins-Gradle}技术栈实现代码质量门禁系统，使用\tech{Groovy}语言开发\tech{Jenkins Job}脚本并集成\tech{ktlint, detekt}等静态检查工具和\tech{Gradle}编译器，确保代码质量合规
  \item[\ding{51}] \highlight{并发架构设计}：设计主从Job并发执行架构，使用多进程并发，提升CI/CD流水线执行效率\textbf{30\%}
  \item[\ding{51}] \highlight{数据管理方案}：基于数据库服务设计检查结果存储和仲裁机制，实现基于\tech{RPC}实现的跨服务器代码检查结果分批记录与集中仲裁，提升门禁系统的灵活性
...
\subsubsection*{科世达车身域控制器CEM项目｜AUTOSAR集成与软件开发负责人}
\hfill \textit{2022年12月 -- 2023年11月}

\begin{itemize}
  \item[\ding{51}] \highlight{AUTOSAR架构集成}：负责车身域控制器的\tech{AUTOSAR CP}架构集成，主导\tech{SWC}间通信配置和基础软件组件集成
...
\begin{itemize}
  ...
  \item[\ding{51}] \highlight{测试工程构建}：构建完整的\tech{vTESTstudio}自动化测试工程，实现从测试用例设计到执行的全流程自动化
