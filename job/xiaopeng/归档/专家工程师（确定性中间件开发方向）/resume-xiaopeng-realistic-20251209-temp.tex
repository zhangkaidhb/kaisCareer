...
\normalsize
5年汽车电子软件开发经验，专注于跨域融合软件架构设计、确定性中间件开发、实时通信系统。精通AUTOSAR \tech{AP}/\tech{CP}架构、\tech{TSN}网络调度、车载以太网通信中间件，具备从架构设计到实际部署的全栈开发能力。

% ============ 关键能力 ============
\section*{关键能力（汽车软件方向）}

\begin{itemize}
  \skillitem{AUTOSAR架构与系统集成}{精通AUTOSAR \tech{CP}架构，熟悉\tech{Adaptive AUTOSAR}，具备HPC平台和车身域控制器集成经验，精通车载网络通信（\tech{CAN}/\tech{LIN}/以太网）}
...
\begin{tabularx}{\textwidth}{@{}p{3.2cm}X@{}}
  \textbf{编程语言} & \tech{Python}, \tech{Groovy}, C, \tech{MATLAB(Stateflow)} \\
  \textbf{Vector工具链} & DaVinci Developer\&Configurator, \tech{CANoe}, \tech{vTESTstudio} \\
  \textbf{CI/CD工具} & \tech{Jenkins}, \tech{Gerrit}, \tech{Gradle}, \tech{MongoDB}, Groovy脚本, \tech{Git} \\
...
\begin{itemize}
  \item[\ding{51}] \highlight{AUTOSAR工具链开发与集成}：负责高性能计算平台的AUTOSAR CP架构集成，主导\tech{yaml2arxml}转换工具和\tech{SWC}配置管理工具开发，实现从yaml脚本配置文件到ARXML和代码的全流程分布式管理自动化，替代\tech{Davinci Developer}核心功能，显著提升综合配置效率
  ...
  \item[\ding{51}] \highlight{系统调试与部署}：通过\tech{Linux}环境配置和劳德巴赫调试器进行C代码级调试，保障系统集成稳定性
\end{itemize}
...
\begin{itemize}
  \item[\ding{51}] \highlight{CI/CD流水线架构与脚本开发}：支持设计企业级CI/CD流水线系统，基于\tech{Gerrit-Jenkins-Gradle}技术栈实现代码质量门禁系统，使用\tech{Groovy}语言开发\tech{Jenkins Job}脚本并集成\tech{ktlint, detekt}等静态检查工具和Gradle编译器，确保代码质量合规
...
\begin{itemize}
  \item[\ding{51}] \highlight{AUTOSAR架构集成}：负责车身域控制器的\tech{AUTOSAR CP}架构集成，主导\tech{SWC}间通信配置和中间件组件集成
