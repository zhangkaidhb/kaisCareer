...
\begin{itemize}
  \item 设计开发stakeholder\_build\_template工具，管理各SWC配置并生成ECUextract和.c .h框架代码
  \item 创建yaml2arxml工具替代Davinci Developer，实现配置文件到AUTOSAR架构的自动转换
  \item 负责ECUExtract\_Unflattened.arxml文件检查，Davinci Developer和Configurator配置消错
  \item 进行SOME/IP服务在Davinci Configurator中的配置和测试
  \item 使用CANoe配合VN5650设备进行车载网络通信测试
  \item 通过PuTTy访问Linux系统，使用劳德巴赫进行断点调试C代码
\end{itemize}
...
\textit{2025年8月 – 至今}
\begin{itemize}
  \item 设计架构Java APP代码门禁pipeline系统，基于Gerrit、Jenkins和Groovy脚本
  \item 在Groovy脚本中集成Gradle的ktlint、detekt进行静态代码检查
  \item 使用主从Job架构实现静态代码检查和Build的并发执行
  \item 设计MongoDB数据库架构实现检查结果的分批记录和集中仲裁
...
\begin{itemize}
  ...
  \item \textbf{CI/CD工具：} Jenkins、Gerrit、Gradle、MongoDB
